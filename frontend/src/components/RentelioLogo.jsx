 ...
 */
export function CycleMark({ className = 'h-[0.92em] w-[0.92em]', color = 'currentColor' }) {
...
export default function RentelioLogo({
  ...
  return (
    <div className={`${centered ? 'text-center' : ''} ${className}`}>
      <div
        className={`font-display font-bold tracking-tight leading-none ${sizes[size] || sizes.md} ${colorClass}`}
        aria-label="Rentelio"
      >
        <span>Renteli</span>
        <CycleMark className="inline-block h-[0.95em] w-[0.95em] mx-[0.02em]" />
      </div>
      {showTagline && (
        <p
          className={`mt-3 font-sans font-medium tracking-[0.04em] text-violet-200/85 ${
            size === 'xl' || size === 'lg' ? 'text-sm md:text-base' : 'text-xs'
          } ${taglineClass}`}
        >
